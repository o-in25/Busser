<script lang="ts">
    import { Label, Fileupload, ButtonGroup, InputAddon, Input, Button, CloseButton} from "flowbite-svelte";
...
<div>
  ...
  <div class="mb-4 flex">
    ...
    <input id={name} {name} type="file" bind:files bind:this={fileInputRef} class="block w-full disabled:cursor-not-allowed disabled:opacity-50 rtl:text-right focus:border-primary-500 focus:ring-primary-500 dark:focus:border-primary-500 dark:focus:ring-primary-500 bg-gray-50 text-gray-900 dark:bg-gray-700 dark:placeholder-gray-400 border-gray-300 dark:border-gray-600 text-sm rounded-lg border !p-0 dark:text-gray-400" />
    {#if hasFiles}
          <CloseButton on:click={clearAll} class="flex absolute inset-y-0 items-center text-gray-500 dark:text-gray-400 end-0 p-2.5" />    {/if}
    </div>
