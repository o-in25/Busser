<script lang="ts">
	...
	import { calculateSuperJuice, getSuperJuiceCitrusOptions } from '$lib/math';
	import type { SuperJuiceCitrus } from '$lib/math';

	const selections = getSuperJuiceCitrusOptions();

	let selectedJuice = $state<SuperJuiceCitrus>('lime');
	let peelWeight = $state('');
	...
	function handleJuiceChange(value: string) {
		selectedJuice = value as SuperJuiceCitrus;
		if (hasCalculated) {
			calculate();
		}
	}
	...
	const calculate = () => {
		const peelWeightGrams = parseFloat(peelWeight) || 0;
		const recipe = calculateSuperJuice(selectedJuice, peelWeightGrams);

		results.citricAcid = recipe.citricAcid.toFixed(2);
		results.malicAcid = recipe.malicAcid.toFixed(2);
		results.malicEnabled = recipe.hasMalic;
		results.msg = recipe.msg.toFixed(2);
		results.msgEnabled = recipe.hasMsg;
		results.water = recipe.waterCups.toFixed(2);

		hasCalculated = true;
	};

	const selectedName = $derived(selections.find((s) => s.value === selectedJuice)?.name || 'Lime');
	const canCalculate = $derived(parseFloat(peelWeight) > 0);
</script>
...
<div class="space-y-6">
	...
	<!-- Calculate Button -->
	<Button variant="primary" onclick={calculate} disabled={!canCalculate} class="w-full sm:w-auto"
