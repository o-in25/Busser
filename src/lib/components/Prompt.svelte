<script lang="ts">
	import { AlertCircle, Loader2, Sparkles, Type, X } from 'lucide-svelte';
		...
		>
			{#if isGenerating}
				<GeneratingOverlay
					...
					subtitle={`AI is describing "${trigger}"`}
			...
			<div class="relative min-h-48" bind:this={textareaWrapper}>
				...
				{#if hasContent && !isGenerating}
					<div
						class="absolute bottom-2 right-3 text-xs text-muted-foreground/60 bg-background/80 dark:bg-white/[0.16] backdrop-blur-sm rounded px-1.5 py-0.5"
						transition:fade={{ duration: 150 }}
					>
