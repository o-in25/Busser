...
<nav class={cn('mb-6 flex flex-col md:flex-row md:items-center gap-2', className)}>
	<div
		class="glass-track relative flex w-full md:w-auto h-10 min-w-0 items-center justify-start gap-1 overflow-x-auto scrollbar-none rounded-xl p-0.5 text-muted-foreground"
	>
		{#each tabs as tab}
			<a
				href={tab.href}
