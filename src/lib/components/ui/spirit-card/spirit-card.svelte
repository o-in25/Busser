...
<label class={cn("cursor-pointer", className)} {...restProps}>
  ...
  >
    ...
    <div class="relative w-full aspect-[4/3] bg-white dark:bg-gray-200 flex items-center justify-center p-2">
      {#if selected}
      ...
      <div class="absolute inset-0 bg-gray-400 mix-blend-multiply opacity-0 dark:opacity-30 pointer-events-none"></div>
    </div>
    ...
    <!-- Selected indicator -->
    {#if selected}
      <div class="absolute top-2 right-2 z-20 w-5 h-5 bg-primary rounded-full flex items-center justify-center shadow-md">
