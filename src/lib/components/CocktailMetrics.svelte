<script lang="ts">
  ...
  import { Percent, Droplets, DollarSign, FlaskConical } from "lucide-svelte";
  ...
  let abv = $derived(() => {
    ...
    const stepsInMl = steps.map(step => ({
      productIdQuantityInMilliliters: convertToMl(step.productIdQuantityUnit, step.productIdQuantityInMilliliters),
...
<div
  class={cn(
    "flex flex-wrap gap-2 p-3 rounded-lg bg-muted/30 border border-border/50",
    className
  )}
  {...restProps}
>
