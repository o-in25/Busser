...
{#if color === 'green'}
<Toast position="top-right" bind:toastStatus={showNotification} color='green' transition={fly} params={{ x: 200 }}  class="mb-4" >
  <svelte:fragment slot="icon">
    <CheckCircleSolid  class="w-5 h-5" />
    <span class="sr-only">Error icon</span>
  </svelte:fragment>
  <div class="ms-3">
    <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Success</h4>
    <div class="text-sm font-normal text-gray-900 dark:text-white">{message}</div>
  </div>
</Toast>
{:else if color === 'red'}
<Toast position="top-right" bind:toastStatus={showNotification} color='red' transition={fly} params={{ x: 200 }} class="mb-4">
  <svelte:fragment slot="icon">
    <CheckCircleSolid  class="w-5 h-5" />
    <span class="sr-only">Error icon</span>
  </svelte:fragment>
...
{:else}
<Toast position="top-right" bind:toastStatus={showNotification} transition={fly} params={{ x: 200 }} color="green" class="mb-4">
  <svelte:fragment slot="icon">
    <CheckCircleSolid  class="w-5 h-5" />
    <span class="sr-only">Error icon</span>
  </svelte:fragment>
