...
export class UserRepository extends BaseRepository {
  ...
  async update(
    ...
    try {
      const user: User = await this.db.query.transaction(async (trx) => {
        await this.db.table('user').where({ userId }).update({ username, email });

        if (roleIds.length) {
          await this.db.table('userRole').where({ userId }).del();
          const userRoles = roleIds.map((roleId) => ({ userId, roleId }));
          await this.db.table('userRole').insert(userRoles);
        }
  ...
  async delete(userId: string, currentUserId: string): Promise<{ error?: string; refresh: User[] }> {
  ...
  async updateGrants(roleId: string, permissions: Permission[]): Promise<QueryResult<Array<Role & Permission>>> {
    try {
      await this.db.query.transaction(async (trx) => {
        ...
        const rolePermissions = oldPermissions.map(({ permissionId }) => ({ roleId, permissionId }));
  ...
  async getInvitationRequests(status?: 'pending' | 'fulfilled' | 'rejected'): Promise<QueryResult<InvitationRequest[]>> {
    try {
  ...
  async fulfillInvitationRequest(invitationRequestId: number, resolvedBy: string): Promise<QueryResult> {
    try {
      const result = await this.db
        .table('invitationRequest')
  ...
  async rejectInvitationRequest(invitationRequestId: number, resolvedBy: string): Promise<QueryResult> {
    try {
      const result = await this.db
        .table('invitationRequest')
        .where({ invitationRequestId })
  ...
  async register(
    ...
    try {
      const { user } = await this.db.query.transaction(async (trx) => {
        ...
        await trx('invitation').update({ userId: user.userId }).where({ invitationId: invitation.invitationId });
      ...
      await this.mailClient.sendUserRegistrationEmail([user.email], { username: user.username, token });
  ...
  async verify(registrationToken: string): Promise<QueryResult> {
    try {
      const { valid, expired, payload } = await this.authRepo.verifyRegistrationToken(registrationToken);
  ...
  async requestPasswordReset(email: string): Promise<QueryResult> {
    try {
      ...
      await this.mailClient.sendPasswordResetEmail([user.email], { username: user.username, token });

      return { status: 'success' };
    } catch (error: any) {
      console.error('Password reset request error:', error);
      return { status: 'error', error: 'Failed to process password reset request. Please try again.' };
    }
  }
}
