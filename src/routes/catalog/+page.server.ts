import { getCatalog, getSpirits, getBasicRecipes } from '$lib/server/core';
...
export const load = (async ({ }) => {
  ...
  const recentCatalog = await getCatalog(1, 6);
  ...
  for (const spirit of spirits) {
    const result = await getBasicRecipes(spirit.recipeCategoryId);
