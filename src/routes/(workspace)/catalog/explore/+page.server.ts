...
export const load = (async ({ parent, locals }) => {
	...
	// Get recipe counts and a spotlight image per spirit category
	const spiritCounts: Record<number, number> = {};
	const spiritSpotlightImages: Record<number, string | null> = {};
	for (const spirit of spirits) {
		...
		if (result.status === 'success' && result.data) {
			...
			const withImage = result.data.find((r) => r.recipeImageUrl);
			spiritSpotlightImages[spirit.recipeCategoryId] = withImage?.recipeImageUrl ?? null;
		} else {
			spiritCounts[spirit.recipeCategoryId] = 0;
			spiritSpotlightImages[spirit.recipeCategoryId] = null;
		}
	}
