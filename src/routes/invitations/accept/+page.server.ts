...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	if (!locals.user) {
		const returnUrl = encodeURIComponent(`/invitations/accept?code=${code}`);
		redirect(StatusCodes.TEMPORARY_REDIRECT, `/login?redirect=${returnUrl}`);
	}

	// check if email matches (if invitation has specific email)
	if (invitation.email && invitation.email.toLowerCase() !== locals.user.email.toLowerCase()) {
		return {
...
export const actions: Actions = {
	accept: async ({ request, locals }) => {
		if (!locals.user) {
			return fail(StatusCodes.UNAUTHORIZED, { error: 'You must be logged in to accept an invitation.' });
		}
		...
		if (!code) {
			return fail(StatusCodes.BAD_REQUEST, { error: 'Invitation code is required.' });
		}
		...
		redirect(StatusCodes.SEE_OTHER, `/recipes?workspace=${result.data?.workspaceId}`);
	},

	decline: async ({ request, locals }) => {
		if (!locals.user) {
			return fail(StatusCodes.UNAUTHORIZED, { error: 'You must be logged in to decline an invitation.' });
		}
