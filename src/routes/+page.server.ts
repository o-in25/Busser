...
export const load = (async ({ locals }) => {
  ...
  let dashboardData: {
    ...
    almostThereRecipes: Array<
      View.BasicRecipe & { missingIngredient: string | null }
    >;
  ...
  if (user && locals.activeWorkspaceId) {
    ...
    const almostThereRecipes =
      await catalogRepo.getAlmostThereRecipes(workspaceId);
    ...
    for (const recipe of recipes) {
      if (recipe.recipeCategoryId !== undefined) {
        spiritCounts[recipe.recipeCategoryId] =
          (spiritCounts[recipe.recipeCategoryId] || 0) + 1;
      }
    }
  ...
  if (!user) {
    ...
    const featuredRecipes = featuredResult.data
      .filter((r) => r.recipeImageUrl)
      .slice(0, 4);
